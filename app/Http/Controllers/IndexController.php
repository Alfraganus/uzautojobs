...
$userExtra = App\ResumeSingleQuestions::where(['user_id'=>$id])->first();


        return view('user.applicant_profile')
            ->with('user', $user)
            ->with('userExtra', $userExtra)
            ->with('profileCv', $profileCv)
            ->with('page_title', $user->getName())
                        ->with('form_title', 'Contact ' . $user->getName());
    }
    
    public function downloadUserCv($id)
    {
        $user = User::findOrFail($id);
        $profileCv = $user->getDefaultCv();
        
        $pdf = PDF::loadView('pdf.user-cv', compact('user'));
        return $pdf->download('cv-'.$id.'.pdf');
    }
    
    public function resume()
    
    {   
        //$resumes = Resume::where('is_default', 1)->orderBy('id', 'desc')->paginate(12);
    
    	$resumes = Resume::orderBy('id', 'desc')->paginate(12);
    	return view('job.resume_list', compact('resumes'));
