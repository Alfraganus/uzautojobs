...
class JobController extends Controller
{
    ...
    {
//        $users = new User();
        $job_appliers = JobApply::orderBy('job_id')->get();

        return view('admin.job.applied_candidates',[
            'job_appliers'=>$job_appliers
        ]);
    }

    public function changeApplicantStatus($job_id)
    {
        $jobStatus = JobApply::where(['id'=>$job_id])->first();
        $statusName = ApplicationStatus::pluck('status_name', 'status_name');
        return view('admin.job.job_status',[
            'jobStatus'=>$jobStatus,
            'statusName'=>$statusName
        ]);
    }
    public function updateChangedStatus($id, Request $request)
    {
//        var_dump($id);die();
        $status = JobApply::where(['id'=>$id])->first();
        $status->application_status = $request->input('application_status');
        $status->save();
        /*         * ************************************ */

        flash('статус был изменен!')->success();
        return \Redirect::route('list.applied.candidates');
    }

/*
    public function fetchApplicants(Request $request)
    {
        $applyjob = JobApply::select([
            'JobApply.application_status', 'JobApply.user_id', 'JobApply.job_id'
        ]);
        return Datatables::of($applyjob)
            ->filter(function ($query) use ($request) {
                if ($request->has('user_id') && !empty($request->company_id)) {
                    $query->where('JobApply.user_id', '=', "{$request->get('user_id')}");
                }
                if ($request->has('title') && !empty($request->title)) {
                    $query->where('JobApply.application_status', 'like', "%{$request->get('application_status')}%");
                }
                if ($request->has('job_id') && !empty($request->title)) {
                    $query->where('JobApply.job_id', 'like', "%{$request->get('job_id')}%");
               }
            })
                ->addColumn('user_id', function ($jobs) {
                    return $applyjob->user->name;
                })
                    ->addColumn('title', function ($jobs) {
                        return $jobs->job->title . ')';
                    })
                    ->addColumn('job_id', function ($jobs) {
                        return $jobs->job->application_status . ')';
                    })
