...
<form class="form" id="add_edit_profile_summary" method="POST" action="{{ route('update.profile.summary', [$user->id]) }}">{{ csrf_field() }}
    <div class="form-body">
        ...
        <div class="form-group" id="div_summary">
            <label for="summary" class="bold">Резюме профиля</label>
        ...
        <button type="button" class="btn btn-large btn-primary" onClick="submitProfileSummaryForm();">Обновить сводку <i class="fa fa-arrow-circle-right" aria-hidden="true"></i></button>
    </div>
</form>
@push('scripts') 
