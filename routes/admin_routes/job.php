...
Route::get('list-applied-candidates', array_merge(['uses' => 'Admin\JobController@appliedCandidates'], $all_users))->name('list.applied.candidates');
Route::get('change-applicant-status/{job_id}', array_merge(['uses' => 'Admin\JobController@changeApplicantStatus'], $all_users))->name('change.applicant.status');
Route::put('update-changed-status/{id}', array_merge(['uses' => 'Admin\JobController@updateChangedStatus'], $all_users))->name('update.changed.status');
